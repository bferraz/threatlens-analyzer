import { ShieldAlert } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Checkbox } from "@/components/ui/checkbox";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
  AccordionTrigger,
} from "@/components/ui/accordion";
import {
  Threat,
  Component,
  STRIDE_LABELS,
  STRIDE_COLORS,
  StrideCategory,
  ThreatCheck,
} from "@/lib/types";

interface ThreatsCardWithChecksProps {
  threats: Threat[];
  components: Component[];
  showSeverity: boolean;
  threatChecks: ThreatCheck[];
  onThreatCheckToggle: (threat: Threat) => void;
  savingCheck?: string | null;
}

const severityColors: Record<string, string> = {
  critical: "severity-critical",
  low: "severity-low",
  medium: "severity-medium",
  high: "severity-high",
};

export function ThreatsCardWithChecks({
  threats,
  components,
  showSeverity,
  threatChecks,
  onThreatCheckToggle,
  savingCheck,
}: ThreatsCardWithChecksProps) {
  const getComponentName = (id: string) => {
    return components.find((c) => c.id === id)?.name || id;
  };

  const getThreatCheck = (threat: Threat) => {
    const threatCheckId = `${threat.targetId}-${threat.category}`;
    return threatChecks.find((c) => c.threatId === threatCheckId);
  };

  // Group threats by component
  const threatsByComponent = threats.reduce(
    (acc, threat) => {
      if (!acc[threat.targetId]) {
        acc[threat.targetId] = [];
      }
      acc[threat.targetId].push(threat);
      return acc;
    },
    {} as Record<string, Threat[]>,
  );

  return (
    <Card className="border-border/50 shadow-md">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-lg">
          <ShieldAlert className="h-5 w-5 text-accent" />
          Threat Model (STRIDE)
          <Badge variant="secondary" className="ml-auto">
            {threats.length}
          </Badge>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Accordion type="multiple" className="space-y-2">
          {Object.entries(threatsByComponent).map(
            ([componentId, componentThreats]) => (
              <AccordionItem
                key={componentId}
                value={componentId}
                className="rounded-lg border border-border/50 bg-muted/30 px-3"
              >
                <AccordionTrigger className="py-3 hover:no-underline">
                  <div className="flex items-center gap-2">
                    <span className="font-medium">
                      {getComponentName(componentId)}
                    </span>
                    <Badge variant="outline" className="text-xs">
                      {componentThreats.length} threats
                    </Badge>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="pb-3">
                  <div className="space-y-2">
                    {componentThreats.map((threat, index) => {
                      const check = getThreatCheck(threat);
                      const threatCheckId = `${threat.targetId}-${threat.category}`;
                      const isChecking = savingCheck === threatCheckId;

                      return (
                        <div
                          key={index}
                          className="rounded-md border border-border/50 bg-card p-3"
                        >
                          <div className="mb-2 flex items-start gap-3">
                            <Checkbox
                              checked={check?.isResolved || false}
                              onCheckedChange={() =>
                                onThreatCheckToggle(threat)
                              }
                              disabled={isChecking}
                              className="mt-0.5"
                            />
                            <div className="flex-1 space-y-2">
                              <div className="flex flex-wrap items-center gap-2">
                                <Badge
                                  variant="outline"
                                  className={`text-xs font-semibold ${STRIDE_COLORS[threat.category as StrideCategory]}`}
                                >
                                  {threat.category} -{" "}
                                  {
                                    STRIDE_LABELS[
                                      threat.category as StrideCategory
                                    ]
                                  }
                                </Badge>
                                {showSeverity && (
                                  <Badge
                                    variant="outline"
                                    className={`text-xs capitalize ${severityColors[threat.severity]}`}
                                  >
                                    {threat.severity}
                                  </Badge>
                                )}
                              </div>
                              <p
                                className={`text-sm font-medium ${check?.isResolved ? "line-through text-muted-foreground" : ""}`}
                              >
                                {threat.title}
                              </p>
                              <p
                                className={`text-xs text-muted-foreground ${check?.isResolved ? "line-through" : ""}`}
                              >
                                {threat.description}
                              </p>
                            </div>
                          </div>
                        </div>
                      );
                    })}
                  </div>
                </AccordionContent>
              </AccordionItem>
            ),
          )}
        </Accordion>
      </CardContent>
    </Card>
  );
}
